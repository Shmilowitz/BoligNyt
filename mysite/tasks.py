def build_bolig_search_url():
    urlformat = "http://www.dba.dk/boliger/ejerbolig/side={side}/".format(i)
    url = []
    for num in range(11):
        next_page = str(num)
    ...
    def get_bolig_details():
        ...
        for url in build_bolig_search_url():
            ...
            for date, price, url, description in map(None,
                ...
                date_found = time
                bolig_records = {'Url': bolig_url, 'date_found': date_found, 'Price': price, 'Description': bolig_description}
...
def save_bolig():
	for boliger in get_bolig_details():
		if not PythonBolig.objects.filter(date_found=boliger['date_found'], Price=boliger['Price'], Url=boliger['Url'], Description=boliger['Description']).exists():
			bolig = PythonBolig(
